...
internal class AutoInsuranceServiceTest {
    ...
    @Autowired
    private lateinit var autoRiskScoreService: AutoInsuranceService

    @Test
    fun `should return ineligible auto risk score`() {
        val riskScore = autoRiskScoreService.plan(0, null)
    ...
    fun `should return economic auto risk score`() {
        val riskScore = autoRiskScoreService.plan(-1, Vehicle(LocalDate.now().year - 1))
    ...
    fun `should return responsible auto risk score`() {
        val riskScore = autoRiskScoreService.plan(2, Vehicle(LocalDate.now().year - 1))
